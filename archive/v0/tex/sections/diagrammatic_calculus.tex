...
\begin{tikzpicture}[scale=0.9]
    ...
    % Three strands: output X (left), X* (middle), input X (right)
    % Cup on LEFT connects output to X*, Cap on RIGHT connects X* to input
    \begin{scope}[xshift=0cm]
        % Output X: exits top-left
        \draw[thick] (0,0.5) -- (0,2.5);
        % Cup at bottom-left: connects output X to X*
        \draw[thick] (0,0.5) arc (180:360:0.4);
        % X* strand: goes up in middle
        \draw[thick] (0.8,0.5) -- (0.8,1.8);
        % Cap at top-right: connects X* to input X
        \draw[thick] (0.8,1.8) arc (180:0:0.4);
        % Input X: enters bottom-right
        \draw[thick] (1.6,0) -- (1.6,1.8);
        % Arrow on input (X going up)
        \draw[thick,->,>=stealth] (1.6,0.2) -- (1.6,0.5);
        % Arrow on output (X going up)
        \draw[thick,->,>=stealth] (0,2.0) -- (0,2.3);
        % Labels
        \node[right] at (1.7,0.3) {$X$};
        \node[left] at (-0.1,2.15) {$X$};
    \end{scope}
    ...
    \begin{scope}[xshift=4cm]
        ...
        \node[left] at (-0.1,0.35) {$X$};
    \end{scope}

    % Label
    \node[right] at (4.8,1.25) {(Zigzag identity)};
\end{tikzpicture}
