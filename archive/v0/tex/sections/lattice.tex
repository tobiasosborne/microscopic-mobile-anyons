...
For an anyonic system described by a fusion category $\mathcal{C}$, the \emph{local Hilbert space} $\mathcal{H}_j$ at site $j$ is identified with the vector space spanned by the simple objects of $\mathcal{C}$ (viewed as possible particle types at that site):
\begin{equation}
    \mathcal{H}_j \cong \bigoplus_{a \in \Irr(\mathcal{C})} \mathbb{C} |a\rangle_j
\end{equation}
where $\Irr(\mathcal{C}) = \{\one=X_0, X_1, \ldots, X_{d-1}\}$ is the set of isomorphism classes of simple objects. The dimension is $d_{\mathrm{loc}} = |\Irr(\mathcal{C})|$, the rank of the category.
\end{definition}

\begin{remark}
This definition naturally accommodates the hard-core constraint where at most one anyon occupies a site. In this context, the ``vacuum'' state corresponds to the unit object $|X_0\rangle_j = |\one\rangle_j$ (no topological charge). For soft-core models allowing multiple anyons per site, the local space would be larger, involving fusion spaces of multiple anyons (see \S\ref{sec:hardcore-softcore}).
\end{remark}
