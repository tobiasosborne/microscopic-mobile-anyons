...
<span id="page-3-0"></span>**Figure 1.** Anyons a and b fuse to anyon c in the  $\mu$ th way.
...
<span id="page-4-0"></span>**Figure 2.** Possible fusion trees for anyons a, b, and c fusing to d.
...
$$F_{e; vy}^{ucd} F_{e; ux}^{aby} = \sum_{v; uw} F_{e; vx}^{abc} F_{e; vx}^{awd} F_{x; wy}^{bcd},$$
(5)
...
<span id="page-7-0"></span>Figure 3. A typical fusion chain.

<span id="page-7-1"></span>**Figure 4.** The *l*th part of a closed chain.
...
$$h_J\left(\frac{\begin{vmatrix} \frac{1}{2} & \frac{1}{2} \\ \end{vmatrix}}{x y z}\right) = J\delta_{xz} \sum_{v} \left(\frac{\sqrt{d_y d_v}}{d_x d_{1/2}} \frac{\begin{vmatrix} \frac{1}{2} & \frac{1}{2} \\ \end{vmatrix}}{x v z}\right). \tag{21}$$
...
<span id="page-21-0"></span>**Figure 7.** (Left) The spectrum of  $H_1$ , k = 3 and l = 14, and the CFT predictions of the  $\mathcal{M}_5$  minimal model. (Right) The spectrum of  $-H_1$ , k = 3 and l = 14, and the CFT predictions of the (Ising  $\times \mathcal{M}_4$ ) conformal field theory.
...
<span id="page-22-0"></span>**Figure 8.** The spectrum of the model Hamiltonian *H*2 for *k* = 2 and the CFT predictions of the Ising CFT. The full blue lines indicate the energies corresponding to the primary fields and the dashed red lines correspond to the descendants.
...
<span id="page-23-0"></span>**Figure 9.** The spectrum of the model Hamiltonian −*H*2 for *k* = 1 with next-nearestneighbor terms (crosses) as well as with only nearest-neighbor terms (dots). The CFT predictions for the *u*(1)8 CFT are indicated by the black lines (primary fields), the red lines (first descendants), and the blue lines (second descendants).
...
The *F*-symbols that appear in the Hamiltonian of our model take the form  $\widetilde{F}_{x;x\pm\frac{1}{2},0}^{x\frac{1}{22}}$ . From equation (A.1), one finds:
