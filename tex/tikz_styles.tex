...
% Vertices are lattice sites, anyons occupy the cells between them
\begin{tikzpicture}[scale=1.2,baseline=(current bounding box.center)]
    % Lattice vertices (small dots)
    ...
    \draw (0,0) -- (3,0);
    \node at (3.4,0) {$\cdots$};
    % Anyon labels in intervals (between vertices)
    % Positioned at midpoints with small brackets to indicate the cell
    \draw[thick,AnyonBlue] (0.1,-0.15) -- (0.1,-0.05) -- (0.9,-0.05) -- (0.9,-0.15);
    \draw[thick,AnyonBlue] (1.1,-0.15) -- (1.1,-0.05) -- (1.9,-0.05) -- (1.9,-0.15);
    \draw[thick,AnyonBlue] (2.1,-0.15) -- (2.1,-0.05) -- (2.9,-0.05) -- (2.9,-0.15);
...
\begin{tikzpicture}[scale=1.2,baseline=(current bounding box.center)]
    ...
    \fill[AnyonBlue,opacity=0.15] (0,-0.12) rectangle (1,0.12);
    \fill[AnyonOrange,opacity=0.15] (1,-0.12) rectangle (2,0.12);
    \fill[AnyonBlue,opacity=0.15] (2,-0.12) rectangle (3,0.12);
    % Lattice vertices (small dots)
    \foreach \x in {0,1,2,3} {
        \fill (\x,0) circle (2pt);
    }
...
% Lines connect exactly to box edges, not through them
\newcommand{\chainlocal}[1]{%
\begin{tikzpicture}[scale=0.6,baseline=(current bounding box.center)]
    \node at (-0.5,0.75) {$\cdots$};
    \draw (0,0) -- (0,1.5);
    % Box spans x=[0.5,1.0], y=[0.45,1.05]
    \node [rectangle,draw,fill=LightGray,minimum width=0.6cm,minimum height=0.5cm,inner sep=2pt] at (0.75,0.75) {\tiny $#1$};
    % Strands touch box edges exactly
    \draw (0.5,0) -- (0.5,0.45);
    \draw (0.5,1.05) -- (0.5,1.5);
    \draw (1,0) -- (1,0.45);
    \draw (1,1.05) -- (1,1.5);
    \draw (1.5,0) -- (1.5,1.5);
    \node at (2,0.75) {$\cdots$};
