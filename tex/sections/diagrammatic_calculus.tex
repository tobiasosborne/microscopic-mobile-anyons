...
\begin{tikzpicture}[scale=0.9]
    ...
    % Shape: X enters from below, CAP bends over left, goes down, CUP bends up
    \begin{scope}[xshift=0cm]
        % Bottom segment: X input going up (right side)
        \draw[thick] (1,0) -- (1,1);
        % CAP: curves UP and over to the left (like going over a hill)
        \draw[thick] (1,1) arc (0:180:0.5);
        % Middle segment: goes DOWN on the left side
        \draw[thick] (0,1) -- (0,2);
        % CUP: curves DOWN then back up to the right (like a valley)
        \draw[thick] (0,2) arc (180:360:0.5);
        % Top segment: X output going up (right side)
        \draw[thick] (1,2) -- (1,3);
        % Arrow on bottom segment (X going up)
        \draw[thick,->,>=stealth] (1,0.3) -- (1,0.6);
        % Arrow on top segment (X going up)
        \draw[thick,->,>=stealth] (1,2.4) -- (1,2.7);
        % Label on input/output side
        \node[right] at (1.1,0.5) {$X$};
        \node[right] at (1.1,2.5) {$X$};
    \end{scope}
    ...
    \begin{scope}[xshift=4.0cm]
        \draw[thick] (0,0) -- (0,3);
        \draw[thick,->,>=stealth] (0,1.3) -- (0,1.7);
        \node[right] at (0.1,1.5) {$X$};
    \end{scope}

    % Label
    \node[right] at (5.0,1.5) {(Zigzag identity)};
\end{tikzpicture}
