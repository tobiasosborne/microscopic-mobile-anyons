...
\begin{tikzpicture}[scale=0.9]
    ...
    % Shape matches nLab reference: S-curve with entry/exit on LEFT
    \begin{scope}[xshift=0cm]
        % Bottom segment: X input going up (left side)
        \draw[thick] (0,0) -- (0,1);
        % CUP: curves DOWN and to the right (bottom of S)
        \draw[thick] (0,1) arc (180:360:0.5);
        % Middle segment: goes up on the right side
        \draw[thick] (1,1) -- (1,2);
        % CAP: curves UP and to the left (top of S)
        \draw[thick] (1,2) arc (0:180:0.5);
        % Top segment: X output going up (left side)
        \draw[thick] (0,2) -- (0,3);
        % Arrow on bottom segment (X going up)
        \draw[thick,->,>=stealth] (0,0.3) -- (0,0.6);
        % Label
    ...
    % Label
    \node[right] at (4.3,1.5) {(Zigzag identity)};
\end{tikzpicture}
