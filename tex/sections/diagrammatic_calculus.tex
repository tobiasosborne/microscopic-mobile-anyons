...
\begin{tikzpicture}[scale=0.9]
    % Zigzag identity: (id_X ⊗ ev) ∘ (coev ⊗ id_X) = id_X
    % Shape: input on right, bend down-left, middle on left, bend down-right, output on right
    \begin{scope}[xshift=0cm]
        % Bottom segment: X input going up (right side)
        \draw[thick] (1,0) -- (1,0.8);
        % First bend: curves down and to the left (like bottom of a cup)
        \draw[thick] (1,0.8) arc (0:-180:0.5);
        % Middle segment: goes up on the left side
        \draw[thick] (0,0.8) -- (0,1.6);
        % Second bend: curves down and to the right (like bottom of a cap)
        \draw[thick] (0,1.6) arc (180:360:0.5);
        % Top segment: X output going up (right side)
        \draw[thick] (1,1.6) -- (1,2.4);
        % Arrow on bottom segment (X going up)
        \draw[thick,->,>=stealth] (1,0.2) -- (1,0.5);
        % Arrow on top segment (X going up)
        \draw[thick,->,>=stealth] (1,1.9) -- (1,2.2);
        % Label
        \node[right] at (1.1,1.2) {$X$};
    \end{scope}
    ...
    \begin{scope}[xshift=4.0cm]
        ...
        \node[right] at (0.1,1.2) {$X$};
    \end{scope}

    % Label
    \node[right] at (5.0,1.2) {(Zigzag identity)};
\end{tikzpicture}
