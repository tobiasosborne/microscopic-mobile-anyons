...
A fusion tree $\tau$ (defined in Definition~\ref{def:fusion-tree}) specifies one of several basis choices for the morphism space $\Mor(X_c, \catO(\mathbf{x}, \mathbf{k}))$. We denote basis states as:
\begin{equation}
    |(\mathbf{x}, \mathbf{k}), \tau, c\rangle \in \catH_N^{(c)}
\end{equation}
where:
\begin{itemize}
    \item $(\mathbf{x}, \mathbf{k})$ is the configuration (positions and anyon types)
...
Given:
\begin{itemize}
    \item Basis states $|\psi\rangle = |(\mathbf{x}, \mathbf{k}), \tau, c\rangle$ and $|\phi\rangle = |(\mathbf{x}', \mathbf{k}'), \tau', c\rangle$ in $\catH_N^{(c)}$
    \item A morphism $f \in \Mor(X_a \otimes X_b, X_c \otimes X_d)$ acting on sites $j, j+1$
\end{itemize}
The matrix element is the complex number:
\begin{equation}
    \langle \phi | f_{j,j+1} | \psi \rangle \in \mathbb{C}
\end{equation}
Interpretation: $f_{j,j+1}$ acts as a morphism between the anyon types at sites $j, j+1$ in the source and target configurations.
\end{definition}
...
\begin{itemize}
    \item Source configuration: $s_j = a, s_{j+1} = b$
    \item Target configuration: $s'_j = c, s'_{j+1} = d$
...
If $f_{ab \to cd}$ is a unitary morphism (or paired with its adjoint), the physical operator $\catO = f_{j,j+1} + f_{j,j+1}^\dagger$ is Hermitian and preserves the sector $\catH_N^{(c)}$.
\end{claim}
...
A morphism $h_R \in \Mor(X_a \otimes \one, \one \otimes X_a)$ describes translating an anyon of type $X_a$ from site $j$ to site $j+1$.
\end{definition}

\textbf{Hard-core configuration support:}
\begin{itemize}
    \item Source: anyon $X_a$ at site $j$, vacuum ($\one$) at site $j+1$
    \item Target: vacuum at site $j$, anyon $X_a$ at site $j+1$
\end{itemize}
...
A morphism $h_L \in \Mor(\one \otimes X_a, X_a \otimes \one)$ describes translating an anyon of type $X_a$ from site $j+1$ to site $j$.
\end{definition}

\textbf{Hard-core configuration support:}
\begin{itemize}
    \item Source: vacuum at site $j$, anyon $X_a$ at site $j+1$
    \item Target: anyon $X_a$ at site $j$, vacuum at site $j+1$
\end{itemize}
...
Let $\{|\psi_i\rangle = |(\mathbf{x}_i, \mathbf{k}_i), \tau_i, c\rangle\}_{i=1}^{d_N^{(c)}}$ be an orthonormal basis of $\catH_N^{(c)}$ (in a chosen fusion tree basis). The matrix representation of a 2-local operator $\catO$ is:
\begin{equation}
...
\begin{itemize}
    \item Type 1 (two-particle interaction) couples configurations with anyons at the same pair of sites $(j, j+1)$; expect $O(n)$ nonzero entries per row.
...
\begin{itemize}
    \item $|\psi_1\rangle = |(1,2), \tau, \one\rangle$ = anyons at sites 1, 2 fusing to $\one$
    \item $|\psi_2\rangle = |(1,3), \tau, \one\rangle$ = anyons at sites 1, 3 fusing to $\one$
    \item $|\psi_3\rangle = |(2,3), \tau, \one\rangle$ = anyons at sites 2, 3 fusing to $\one$
\end{itemize}

Right-hopping operator $h_{R,1,2}$ on sites 1, 2 (moves anyon from site 1 to site 2):
\begin{itemize}
    \item Source: anyon at site 1, vacuum at site 2 $\to$ only $|\psi_2\rangle = |(1,3)\rangle$ qualifies
    \item Target: vacuum at site 1, anyon at site 2 $\to$ only $|\psi_3\rangle = |(2,3)\rangle$ qualifies
    \item Non-zero element: $\langle \psi_3 | h_{R,1,2} | \psi_2 \rangle$
\end{itemize}
