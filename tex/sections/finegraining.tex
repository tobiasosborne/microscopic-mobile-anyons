...
\begin{tabular}{lcc}
\hline
\textbf{Construction} & \textbf{Single species} & \textbf{Multi-species} \\
\hline
Product map (Haar) & \checkmark & \checkmark \\
Product map (D4) & $\times$ & $\times$ \\
Slater $\Lambda^N(R) \otimes I$ & \checkmark & $\times$ \\
Normalized product & \checkmark & \checkmark \\
\hline
\end{tabular}
\caption{Fine-graining isometry constructions. The normalized product map~\eqref{eq:normalized-product} is the universal construction that works in all cases.}
\label{tab:hierarchy}
\end{table}
